...
  
  // KORREKTUR: Verwende Optional Chaining (?.) um auf tracking zuzugreifen und 
  // nullish coalescing (??) um einen leeren String oder null als Fallback zu verwenden.
  const [selectedRegion, setSelectedRegion] = useState(campaign.tracking?.currentRegionId ?? null);

  const handleAdvanceTime = async () => {
    // Zusätzliche Sicherheitsprüfung, obwohl die übergeordnete Komponente dies steuern sollte.
...
        ...campaign,
        // tracking ist bereits auf Existenz geprüft
        tracking: {
            ...campaign.tracking,
...
    // Zusätzliche Sicherheitsprüfung, falls campaign.tracking fehlt
    if (!campaign.tracking) return;
...
  };
  ...
  // Zusätzliche Sicherheitsprüfung, falls campaign.tracking fehlt.
  // Das sollte aufgrund der Logik in der übergeordneten Komponente nicht passieren, 
  // ist aber sicherer.
  if (!campaign.tracking) {
    return (
        <Card className="mb-8 p-6 text-center text-muted-foreground border-dashed">
            {t('Tracking data is not available for this campaign.')}
        </Card>
    );
  }
