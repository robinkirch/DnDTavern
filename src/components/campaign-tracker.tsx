...
import { updateCampaign, updateCampaignTracking } from '@/lib/data-service';
...
export function CampaignTracker({ campaign, setCampaign }: CampaignTrackerProps) {
  ...
  const handleAdvanceTime = async () => {
    ...
    try {
      await updateCampaignTracking(campaign.id, newTracking);
      ...
      toast({ variant: 'destructive', title: t('Error'), description: t('Failed to save the campaign settings.') });
    }
  };
  ...
  const newTracking = { ...campaign.tracking, currentRegionId: regionId };
